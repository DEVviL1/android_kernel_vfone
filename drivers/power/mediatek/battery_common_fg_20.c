...
#ifdef CONFIG_BATT_ID_CHECK_SUPPORT
static ssize_t show_Battery_Id(struct device *dev,struct device_attribute *attr, char *buf)
{
    printk( "[EM] Battery_ID : %d\n", BMT_status.id_vol);
#ifndef TARGET_BUILD_MMITEST
    return sprintf(buf, "%d\n", BMT_status.id_vol);
#else
	if((BMT_status.id_vol > CHG_MIN_VALID_BATT_ID_SCUD)&&(BMT_status.id_vol < CHG_MAX_VALID_BATT_ID_SCUD))
            return sprintf(buf, "%d\n", BMT_status.id_vol);
	else
...
int battery_meter_get_battery_id_voltage(void)
{
...
void mt_battery_GetBatteryData(void)
{
	...
	if (upmu_is_chr_det() == KAL_TRUE) {
		...
		battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CURRENT, &ICharging);
		ICharging = ICharging / 10;
...
extern unsigned int ctp_source_num;
...
void do_chrdet_int_task(void)
{
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			BMT_status.charger_exist = KAL_TRUE;
            //printk("do_chrdet_int_task 111 : ctp_source_num= %d.\n",ctp_source_num);	
            if(ctp_source_num ==1)
				tpd_usb_plugin(1);
			else if(ctp_source_num ==2)
			...
			BMT_status.charger_exist = KAL_FALSE;
			//printk("do_chrdet_int_task 222 : ctp_source_num= %d.\n",ctp_source_num);	
			if(ctp_source_num ==1)
				tpd_usb_plugin(0);
			else if(ctp_source_num ==2)
...
void BAT_thread(void)
{
	...
	#ifdef TARGET_BUILD_MMITEST
		battery_meter_ctrl(BATTERY_METER_CMD_GET_HW_FG_CURRENT, &ICharging);
		BMT_status.ICharging = ICharging / 10;
	#endif
	}
...
#endif
	battery_charging_control(CHARGING_CMD_REGISTER_RESET, NULL);
