...
struct of_device_id touch_of_match[] = {
	...
	{ .compatible = "mediatek,shine_plus_touch", },
	{},
};
	...
	if (node1) {
		...
		if (tpd_dts_data.use_tpd_button) {
			...
			of_property_read_u32_array(node1, "tpd-key-local",
				tpd_dts_data.tpd_key_local, tpd_dts_data.tpd_key_num);
	...
	for (i = 1; i < TP_DRV_MAX_COUNT; i++) {
		...
		if (tpd_driver_list[i].tpd_device_name != NULL) {
			...
			if (tpd_load_status == 1) {
				...
				if (strcmp(g_tpd_drv->tpd_device_name, "ft5436") == 0)
			    ctp_source_num = 1;
			    else
				ctp_source_num = 2;
