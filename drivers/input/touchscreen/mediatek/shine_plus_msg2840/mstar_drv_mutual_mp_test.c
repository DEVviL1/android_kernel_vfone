...
static s32 _gDeltaC_2[MAX_MUTUAL_NUM] = {0};
...
{        
    ...
    for (i = 0; i < nSenNumBak; i++)
    {
        for (j = 0; j < nDrvNumBak; j++)
        {
            ...
            if (_gTpType == TP_TYPE_JD)
            {
                nDrvPos = g_MapVaMutual_JD[nShift][1];            
                nSenPos = g_MapVaMutual_JD[nShift][0];            
            }
...
static void _DrvMpTestItoTestMsg28xxAnaEnableChargePump(u8 enable_charge_pump)
{
    u16 val = 0;
...
static void _DrvMpTestItoTestMsg28xxUpdateAnaChargeDumpSetting(void)
{
...
static void _DrvMpTestItoOpenTestMsg28xxAndChangeCDtime(u16 nTime1, u16 nTime2)
{
    DBG("*** %s() ***\n", __func__);
...
static s32 _DrvMpTestMsg28xxItoOpenTest(void)
{
	...
	_DrvMpTestItoOpenTestMsg28xxAndChangeCDtime(0x000C, 0x0006); //for open charge/down
	_DrvMpTestItoTestMsg28xxAnaEnableChargePump(0);
    RegSet16BitValue(0x156A, 0x000A); ///DAC com voltage
    _DrvMpTestItoTestMsg28xxUpdateAnaChargeDumpSetting();
...
static u16 _DrvMpTestMsg28xxItoTestChooseTpType(void)
{
    ...
    if (TP_TYPE_JD == _gTpType) // Modify. 
    {
        ...
        SENSE_NUM = SENSE_NUM_JD;
        DRIVE_NUM = DRIVE_NUM_JD;
        KEY_NUM = KEY_NUM_JD;
        KEY_LINE = KEY_LINE_JD;
        GR_NUM = GR_NUM_JD;
        CSUB_REF = CSUB_REF_JD;
        SENSE_MUTUAL_SCAN_NUM = SENSE_MUTUAL_SCAN_NUM_JD;
        MUTUAL_KEY = MUTUAL_KEY_JD;
        PATTERN_TYPE = PATTERN_TYPE_JD;
    
        SHORT_N1_TEST_NUMBER = SHORT_N1_TEST_NUMBER_JD;
        SHORT_N2_TEST_NUMBER = SHORT_N2_TEST_NUMBER_JD;
        SHORT_S1_TEST_NUMBER = SHORT_S1_TEST_NUMBER_JD;
        SHORT_S2_TEST_NUMBER = SHORT_S2_TEST_NUMBER_JD;
        SHORT_TEST_5_TYPE = SHORT_TEST_5_TYPE_JD;
        SHORT_X_TEST_NUMBER = SHORT_X_TEST_NUMBER_JD;
                                                                                                                                            
        SHORT_N1_TEST_PIN = MSG28XX_SHORT_N1_TEST_PIN_JD;
        SHORT_N1_MUX_MEM_20_3E = SHORT_N1_MUX_MEM_20_3E_JD;
        SHORT_N2_TEST_PIN = MSG28XX_SHORT_N2_TEST_PIN_JD;
        SHORT_N2_MUX_MEM_20_3E = SHORT_N2_MUX_MEM_20_3E_JD;
        SHORT_S1_TEST_PIN = MSG28XX_SHORT_S1_TEST_PIN_JD;
        SHORT_S1_MUX_MEM_20_3E = SHORT_S1_MUX_MEM_20_3E_JD;
        SHORT_S2_TEST_PIN = MSG28XX_SHORT_S2_TEST_PIN_JD;
        SHORT_S2_MUX_MEM_20_3E = SHORT_S2_MUX_MEM_20_3E_JD;
        SHORT_X_TEST_PIN = MSG28XX_SHORT_X_TEST_PIN_JD;
        SHORT_X_MUX_MEM_20_3E = SHORT_X_MUX_MEM_20_3E_JD;                                                           
                                       
        PAD_TABLE_DRIVE = PAD_TABLE_DRIVE_JD;
        PAD_TABLE_SENSE = PAD_TABLE_SENSE_JD;
        PAD_TABLE_GR = PAD_TABLE_GR_JD;
...
#if 0
 static void patchFWAnaSetting_for_shorttest(void)
{
            u8 i = 0;

            // overwrite sensor PAD , restore to default state
            for (i = 0; i < 8; i++)
                 RegSet16BitValue( 0x1e33 + i, 0x0000);

            // overwrite PAD gpio , restore to default state
             RegSet16BitValue( 0x1e30, 0x000f);
             RegSet16BitValue( 0x1e31, 0x0000);
             RegSet16BitValue( 0x1e32, 0xffff);
             RegSet16BitValue( 0x1e3b, 0xffff);
             RegSet16BitValue( 0x1e3c, 0xffff);

             RegSet16BitValue( 0x1e3d, 0x003f);
            for (i = 0; i < 20; i++)
                 RegSet16BitValue( 0x2110 + i, 0x0000);
            for (i = 0; i < 16; i++)
                 RegSet16BitValue( 0x2160 + i, 0x0000);
...
static s32 _DrvMpTestMsg28xxItoShortTest(u8 nItemID)
{
  ...
  //  _DrvMpTestItoShortTestMsg28xxAndChangeCDtime(0x007E, 0x001F);    
    _DrvMpTestItoShortTestMsg28xxAndChangeCDtime(0x007E, 0x0006);     //sway2016

    ///DAC overwrite
   // RegSet16BitValue(0x150C, 0x80A2); //bit15 //AE:3.5v for test
     RegSet16BitValue(0x150C, 0x8040); //bit15 //AE:1.3v for test
...
/*
    /// AFE gain = 1X    sway2016
    RegSet16BitValue(0x1318, 0x4440);
    RegSet16BitValue(0x131A, 0x4444);

    	     RegSet16BitValue( 0x1012, 0x0680);
    	     RegSet16BitValue( 0x1022, 0x0000);
 	     RegSet16BitValue( 0x110A, 0x0104);
            RegSet16BitValue( 0x1310, 0x04F1);
            RegSet16BitValue( 0x1317, 0x04F1);
            RegSet16BitValue( 0x1432, 0x0000);
            RegSet16BitValue( 0x1435, 0x0C00);
            RegSet16BitValue( 0x1538, 0x0400);//
            RegSet16BitValue( 0x1540, 0x0012);//
            RegSet16BitValue( 0x1530, 0x0133);//HI v buf enable
            RegSet16BitValue( 0x1533, 0x0522);//low v buf gain
            RegSet16BitValue( 0x1E11, 0x8000);
            RegSet16BitValue( 0x2003, 0x007E);
            RegSet16BitValue( 0x2006, 0x137F);
            RegSet16BitValue( 0x213E, 0x1FFF);

// re-set sample and coefficient   sway2016
            RegSet16BitValue( 0x100D, 0x0020);
            RegSet16BitValue( 0x1103, 0x0020);
            RegSet16BitValue( 0x1104, 0x0020);
            RegSet16BitValue( 0x1302, 0x0020);
            RegSet16BitValue( 0x1B30, 0x0020);
 ...
 */
   ...
   // nRegData = RegGetLByteValue(0x101A);
   nRegData=0x0020;                           //sway2016
    nAfeCoef = 0x10000 / nRegData;
    //RegSet16BitValue( (uint)0x13D6, AFE_coef);
    RegSet16BitValue(0x13D6, nAfeCoef);
	...
	_ItoTestDebugShowArray(_gDeltaC, 65, -32, 10, 13);
	

       RegSet16BitValue(0x150C, 0x8083); ////AFE:3.72v for test
    if (_DrvMpTestItoShortTestMsg28xxGetValueR(_gDeltaC_2)<0)
    {
        DBG("*** Msg28xx Short Test# GetValueR failed! ***\n");      
        return -1;
    }
	_ItoTestDebugShowArray(_gDeltaC_2, 65, -32, 10, 13);

    for (i = 0; i < 65; i++) // 13 AFE * 5 subframe
    {
    	if ((_gDeltaC_2[i] < IIR_MAX) && (_gDeltaC[i] < IIR_MAX))
    		_gDeltaC[i] = _gDeltaC_2[i] - _gDeltaC[i];
    	else
    		_gDeltaC[i] = _gDeltaC_2[i];
...
//	return ((3.72 - 1.3) * 2.15 * 32768 * 1.1 / (50 * ((double)nValue - 0)));

	return (572*32768)/(5000*nValue);
}
