...
#include <mt_gpio.h> //task672921 modify by zhanyong.yin at 20150928
...
#define GPIO_CAMERA_FLASH_FRONT_EN (GPIO82 | 0x80000000) //task672921 modify by zhanyong.yin at 20150928
...
static int FL_Enable(void)
{
	...
	if(g_duty<1)
	{
		mt_set_gpio_out(GPIO_CAMERA_FLASH_FRONT_EN,GPIO_OUT_ONE);
...
{
    mt_set_gpio_out(GPIO_CAMERA_FLASH_FRONT_EN,GPIO_OUT_ZERO);
...
{
    /*Init. to disable*/
    if(mt_set_gpio_mode(GPIO_CAMERA_FLASH_FRONT_EN,GPIO_MODE_00)){PK_DBG("[sub_flashlight] set gpio mode failed!! \n");}
    if(mt_set_gpio_dir(GPIO_CAMERA_FLASH_FRONT_EN,GPIO_DIR_OUT)){PK_DBG("[sub_flashlight] set gpio dir failed!! \n");}
    if(mt_set_gpio_out(GPIO_CAMERA_FLASH_FRONT_EN,GPIO_OUT_ZERO)){PK_DBG("[sub_flashlight] set gpio failed!! \n");}
