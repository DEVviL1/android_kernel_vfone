/* drivers/hwmon/mt6516/amit/epl257x.c - EPL257x ALS/PS driver
 *
 ...
 * driver info
*******************************************************************************/
#define EPL_DEV_NAME   		    "EPL257x"
#define DRIVER_VERSION          "3.0.0_TCL"
/******************************************************************************
...
#define LUX_PER_COUNT	1200  //ALS lux per count
/*ALS interrupt table*/
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
    ...
    epl_sensor.als.cycle = EPL_CYCLE_64;//EPL_CYCLE_16;
    ...
    epl_sensor.als.report_type = CMC_BIT_DYN_INT; //CMC_BIT_DYN_INT;
    ...
    dynk_low_offset = 550;//500;
    dynk_high_offset = 1400;//800;
#endif /*PS_DYN_K*/
...
/*configure to GPIO function, external interrupt*/
	if (epl_sensor_obj->irq_node)
	{
		...
		if (request_irq(epl_sensor_obj->irq, epl_eint_handler, IRQF_TRIGGER_NONE, "ALS-eint", NULL)) {
...
static ssize_t epl_sensor_show_mmi_psdata(struct device_driver *ddri, char *buf)
{
    ssize_t len = 0;
    int ps_raw, high_threshold, low_threshold;

    ps_raw = factory_ps_data();
    high_threshold = ps_raw + 625; 
    low_threshold = ps_raw + 235;
    APS_LOG("[%s]: ps_raw=%d \r\n", __func__, ps_raw);
    len += snprintf(buf + len, PAGE_SIZE - len, "%d:%d:%d", ps_raw, high_threshold, low_threshold);
...
static DRIVER_ATTR(mmi_psdata,   S_IWUSR  | S_IRUGO, epl_sensor_show_mmi_psdata,NULL);
...
{
	const char *name = "mediatek,epl257x";
	printk("%s\n", __func__);
...
MODULE_DESCRIPTION("EPL257x ALPsr driver");
MODULE_LICENSE("GPL");
