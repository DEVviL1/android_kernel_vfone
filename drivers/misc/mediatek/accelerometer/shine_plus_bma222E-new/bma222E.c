...
#define  SENSOR_TYPE_BMA222E    BMA222E_TYPE
...
static const struct bma222e_type_map_t g_sensor_type_map[] = {
...
static int BMA222_CheckDeviceID(struct i2c_client *client)
{
	...
	GSE_LOG("BMA222_CheckDeviceID %d done, the sensor is %s\n ",
			databuf[0], g_sensor_type_map[obj->sensor_type].sensor_name);

	GSE_LOG("BMA222_CheckDeviceID sensor_type=%d!\n",obj->sensor_type);
...
static int BMA222E_MatchDeviceID(struct i2c_client *client)
{
//	u8 databuf[2]={0};    
//	int res = 0;
	struct bma222_i2c_data *obj = i2c_get_clientdata(client);

	GSE_LOG("BMA222E_MatchDeviceID sensor_type=%d!\n",obj->sensor_type);

    if(obj->sensor_type==SENSOR_TYPE_BMA222E)
        return BMA222_SUCCESS;
	else
		return BMA222_ERR_I2C;
}
...
static int bma222_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = BMA222E_MatchDeviceID(client); 
	if(res != BMA222_SUCCESS)
	{
		return res;
	}	
	printk("BMA222E_MatchDeviceID ok\n"); 
...
static int BMA222_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
    ...
    //printk("ori x=%d,y=%d,z=%d,sensitivity =%d",obj->data[BMA222_AXIS_X],obj->data[BMA222_AXIS_Y],obj->data[BMA222_AXIS_Z],obj->reso->sensitivity);
