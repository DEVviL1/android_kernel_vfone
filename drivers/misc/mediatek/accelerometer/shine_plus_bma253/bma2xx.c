...
#define  SENSOR_TYPE_BMA253    BMA255_TYPE
...
struct acc_hw accel_cust_bma253;
static struct acc_hw *hw = &accel_cust_bma253;
...
static int BMA2XX_CheckDeviceID(struct i2c_client *client)
{
	...
	GSE_LOG("BMA2XX_CheckDeviceID sensor_type=%d!\n",obj->sensor_type);
...
static int BMA2XX_MatchDeviceID(struct i2c_client *client)
{
//	u8 databuf[2]={0};    
//	int res = 0;
	struct bma2xx_i2c_data *obj = i2c_get_clientdata(client);

    GSE_LOG("BMA2XX_MatchDeviceID sensor_type=%d!\n",obj->sensor_type);
	
    if(obj->sensor_type==SENSOR_TYPE_BMA253)
        return BMA2XX_SUCCESS;
	else
		return BMA2XX_ERR_I2C;
}
...
static int bma2xx_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = BMA2XX_MatchDeviceID(client); 
	if(res != BMA2XX_SUCCESS)
	{
		return res;
	}	
	printk("BMA2XX_MatchDeviceID ok \n");
...
static int  bma2xx_local_init(void)
{
	...
	BMA2XX_power(hw, 1);
	GSE_LOG("bma2xx_local_init  111\n");
	if(i2c_add_driver(&bma2xx_i2c_driver))
	{
		GSE_ERR("add driver error\n");
		return -1;
	}
	GSE_LOG("bma2xx_local_init  222\n");
