			   ...
			   enum led_brightness level)
{
	...
	if (strcmp(led_data->cust.name, "red") == 0) {
		mt_set_gpio_mode(GPIO24, GPIO_MODE_00);
...
unsigned int breath_blink_mode = 0;
static ssize_t store_blink_mode(struct device *dev,
				  struct device_attribute *attr,
				  const char *buf, size_t size)
{
	int rc;
	...
	if (buf != NULL && size != 0) {
		rc = kstrtouint(buf, 10, &breath_blink_mode);
		if(rc < 0)
			LEDS_DRV_DEBUG
				    ("kstrtouint failed\n");
		LEDS_DRV_DEBUG
...
static DEVICE_ATTR(blink_mode, 0664, show_blink_mode, store_blink_mode);
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		if (cust_led_list[i].mode == MT65XX_LED_MODE_GPIO) {
			mt_set_gpio_mode(GPIO56, GPIO_MODE_00);
			mt_set_gpio_dir(GPIO56, GPIO_DIR_OUT);
