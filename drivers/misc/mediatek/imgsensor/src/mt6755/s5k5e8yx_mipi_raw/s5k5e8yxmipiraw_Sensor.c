...
//#define LOG_1 printk("s5k5e8yx,MIPI 2LANE\n")
//#define LOG_2 printk("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n")
/****************************   Modify end    *******************************************/
//#define printk(format, args...)	printk("ANDROID_printkO", PFX, "[%s] " format, __FUNCTION__, ##args)

#define SHADING_ONOFF 0x00   /*0x00 on; 0x01 off*/ 
#define S5k5e8_OTP_CALI

#define DISABLE_LSC_IN_LL
#ifdef DISABLE_LSC_IN_LL /*Disable LSC in low light condition*/
#define LL_SHUTTER	0x1ddf /*low light shutter value*/
...
static void write_shutter(kal_uint16 shutter)
{
	...
	printk("hujl@debug:shutter value = 0x%x\n", shutter);

	#ifdef DISABLE_LSC_IN_LL
	if (shutter >= LL_SHUTTER) {
	
 		write_cmos_sensor(0x341A,0x00);
 		write_cmos_sensor(0x341B,0x00);
 		write_cmos_sensor(0x341C,0x10);
 		write_cmos_sensor(0x341D,0x00);
 		write_cmos_sensor(0x341E,0x00);
 		write_cmos_sensor(0x341F,0x00);
 		write_cmos_sensor(0x3420,0x00);
 		write_cmos_sensor(0x3421,0x00);  //  END
 		
 		
		write_cmos_sensor(0x3412,0x06);  //add for lsc dark corner reddish
		write_cmos_sensor(0x3413,0x5E);
 

		
	} else if (shutter < LL_SHUTTER) {
		

 		write_cmos_sensor(0x3412,0x00);  //add for lsc dark corner reddish
...
static void preview_setting(void)
{
	...
	//Delay 1 frame
	mDELAY(33);    
	//Exhujl@debug:TCLk_frequency_mhz
	...
	write_cmos_sensor(0x3C14,0x04); //reg_ouhujl@debug:TCLk_pll_clk_sel[2], ouhujl@debug:TCLk_pll_exhujl@debug:TCLk_sel[1], main_pll_exhujl@debug:TCLk_sel[0]
	...
	write_cmos_sensor(0x3400,SHADING_ONOFF); //shade_bypass LSC on
	//Streaming on
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150){
		...
		write_cmos_sensor(0x0203,0XDE);
		write_cmos_sensor(0x3303,0X02);
		write_cmos_sensor(0x3400,SHADING_ONOFF);
		write_cmos_sensor(0x3C0D,0X04);
		write_cmos_sensor(0x0100,0X01);
		write_cmos_sensor(0x3C22,0X00);
		write_cmos_sensor(0x3C22,0X00);
...
static void read_OTP(void)
{
	...
	if(page_flag==0x01){
		printk("hujl@debug:TCL s5k5e8 OTP select page 14\n");
		...
		printk("hujl@debug:TCL s5k5e8 OTP module_id is 0x%x,lens_id is 0x%x,vcm_id is 0x%x,driver_ic_id is 0x%x\n",module_id,lens_id,vcm_id,driver_ic_id);
		printk("hujl@debug:TCL s5k5e8 OTP date is %d-%d-%d\n",year,month,date);
		...
		printk("hujl@debug:TCL s5k5e8 OTP rg_ratio is 0x%x,bg_ratio is 0x%x\n",rg_ratio,bg_ratio);
		printk("hujl@debug:TCL s5k5e8 OTP g_rg_ratio is 0x%x,g_bg_ratio is 0x%x\n",g_rg_ratio,g_bg_ratio);
		r_ratio = 512 * g_rg_ratio /rg_ratio;
    	      	b_ratio = 512 * g_bg_ratio /bg_ratio;

	     	printk("hujl@debug:TCL s5k5e8 OTP : r_ratio=%d  b_ratio=%d\n",r_ratio,b_ratio);
		if((module_id==0xC)&&(lens_id==0x72)&&(vcm_id==0x0)&&(driver_ic_id==0x0))
		{
			
	     		printk("hujl@debug:TCL s5k5e8 OTP : OTP DATA is valid!\n");
			OTP_vailiable=1;
		} else{
	     		printk("hujl@debug:TCL s5k5e8 OTP : OTP DATA is not valid!\n");
			OTP_vailiable=0;
		}

		if((module_id==0xC)&&(lens_id==0x72))
		{
	...
	if(page1_flag==0x01){
	printk("hujl@debug:TCL s5k5e8 OTP select page 15\n");
	module_id=read_cmos_sensor(0x0A04);
	year=read_cmos_sensor(0x0A05);
	month=read_cmos_sensor(0x0A06);
	date=read_cmos_sensor(0x0A07);
	...
	OTP_vailiable=0;
	OTP_vailiable_mini=0;
	printk("hujl@debug:TCL s5k5e8 no OTP");
	}
...
static void set_otp_awb(kal_uint16 R_Gain ,kal_uint16 B_Gain,kal_uint16 G_Gain)
{
	...
	printk("hujl@debug:TCL:[S5K5E8Y] [S5K5E2Y_MIPI_read_otp_wb] R_GainH=0x%x\n", R_GainH);	
	printk("hujl@debug:TCL:[S5K5E8Y] [S5K5E2Y_MIPI_read_otp_wb] R_GainL=0x%x\n", R_GainL);	
	printk("hujl@debug:TCL:[S5K5E8Y] [S5K5E2Y_MIPI_read_otp_wb] B_GainH=0x%x\n", B_GainH);
	printk("hujl@debug:TCL:[S5K5E8Y] [S5K5E2Y_MIPI_read_otp_wb] B_GainL=0x%x\n", B_GainL);	
	printk("hujl@debug:TCL:[S5K5E8Y] [S5K5E2Y_MIPI_read_otp_wb] G_GainH=0x%x\n", G_GainH);	
	printk("hujl@debug:TCL:[S5K5E8Y] [S5K5E2Y_MIPI_read_otp_wb] G_GainL=0x%x\n", G_GainL);
#if 1
...
//S5K5E2YA_wordwrite_cmos_sensor(GAIN_GREEN2_ADDR, GAIN_DEFAULT); //Green 2 default gain 1x
    ...
    printk("hujl@debug:TCL: WB Gain Set [R_GAIN=%d],[G_GAIN=%d],[B_GAIN=%d] \n",R_GAIN, G_GAIN, B_GAIN);
    set_otp_awb(R_GAIN,B_GAIN,G_GAIN);
}

static void apply_lsc_otp(void)
{
        printk("hujl@debug:TCL:S5K5E8 OTP : LSC Auto Correct \n");
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
		...
		case SENSOR_FEATURE_GET_CROP_INFO:
            //printk("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%ld\n", *feature_data);
