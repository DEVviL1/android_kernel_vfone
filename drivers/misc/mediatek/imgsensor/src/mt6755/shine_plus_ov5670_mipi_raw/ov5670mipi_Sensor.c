...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.mipi_data_lp2hs_settle_dc = 65,//85,//unit , ns
...
static void preview_setting(void)
{
	...
	write_cmos_sensor(0x3820, 0x96);  //90 							  
	write_cmos_sensor(0x3821, 0x41);  //47 							  
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 150) { //15fps for PIP
		...
		write_cmos_sensor(0x3820, 0x86);//80 
		write_cmos_sensor(0x3821, 0x40); //46
...
static int ov5670_read_otp(void)
{
	...
	int awb_flag,sensor_id;
	int temp1,module_id,lens_id;
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {	
				printk("ov5670 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				sprintf(Front_Camera_Name,"OV5670:E-WELLY:SUP1:5M_FF");
					return ERROR_NONE;
				}
			 printk("ov5670 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			retry--;
		} while(retry > 0);
		i++;
		retry = 2;
