...
#define OV5675_OTP_CALI

#ifdef OV5675_OTP_CALI
#define Golden_RG	0x261
#define Golden_BG	0x291

#ifndef ABS
#define ABS(a) ((a) > 0 ? (a) : -(a))
#endif
static int otp_flag=0,otp_awb_flag=0,mid=0,lens_id=0,vcm_id=0;//check_awb_data=0;
#endif
...
{{ 2624, 2000,	  0,   12, 2624, 1976, 1312,  988, 0000, 0000, 1312,   988,	  8,	8, 1296,  972}, // Preview 
...
static struct otp_struct ov5675_otp_data;
...
//	int temp1=0,temp2=0,temp3=0;
	...
	if(addr != 0) {
		...
		(*otp_ptr).vcm_id = read_cmos_sensor(addr + 2);
		(*otp_ptr).vcm_ic_id = read_cmos_sensor(addr + 3);
		(*otp_ptr).production_year = read_cmos_sensor(addr + 4);
		(*otp_ptr).production_month = read_cmos_sensor(addr + 5);
		(*otp_ptr).production_day = read_cmos_sensor(addr + 6);
	} else {
	...
	printk("TCL ov5675 otp info otp_flag:0x%x,module_integrator_id:0x%x,\
		lens_id:0x%x,vcm_id:0x%x,vcm_ic_id:0x%x,date is %d-%d-%d\n",
		otp_flag,(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,
		(*otp_ptr).vcm_id,(*otp_ptr).vcm_ic_id,(*otp_ptr).production_year,
		(*otp_ptr).production_month,(*otp_ptr).production_day);

	mid = (*otp_ptr).module_integrator_id;
	lens_id = (*otp_ptr).lens_id;
	vcm_id = (*otp_ptr).vcm_id;

	// OTP WB Calibration
	otp_awb_flag = read_cmos_sensor(0x7026);
	addr = 0;
	if((otp_awb_flag & 0xc0) == 0x40) {
		addr = 0x7027;// base address of WB Calibration group 1
	} else if((otp_awb_flag & 0x30) == 0x10) {
		addr = 0x7030;// base address of WB Calibration group 2
	} else if((otp_awb_flag & 0x0c) == 0x04) {
		addr = 0x7039;// base address of WB Calibration group 3
	}

	printk("[ov5675 otp read] addr = %x\n", addr);

	if(addr != 0) {
		(*otp_ptr).flag |= 0x40;
		temp = read_cmos_sensor(addr + 4);
		temp1 = read_cmos_sensor(addr + 8);
		(*otp_ptr).rg_ratio = (read_cmos_sensor(addr + 1)<<2) + ((temp>>6) & 0x03);
		(*otp_ptr).bg_ratio = (read_cmos_sensor(addr + 2)<<2) + ((temp>>4) & 0x03);
		(*otp_ptr).g_rg_ratio = (read_cmos_sensor(addr + 5)<<2) + ((temp1>>6) & 0x03);
		(*otp_ptr).g_bg_ratio = (read_cmos_sensor(addr + 6)<<2) + ((temp1>>4) & 0x03);
	} else {
		(*otp_ptr).rg_ratio = 0;
		(*otp_ptr).bg_ratio = 0;
		(*otp_ptr).g_rg_ratio = 0;
		(*otp_ptr).g_bg_ratio = 0;
	}

#if 0
	temp2=ABS(((*otp_ptr).rg_ratio)*1000/((*otp_ptr).g_rg_ratio)-1000);
	temp3=ABS(((*otp_ptr).bg_ratio)*1000/((*otp_ptr).g_bg_ratio)-1000);
	printk("[ov5675 otp]temp2=%d,temp3=%d\n",temp2,temp3);

	if((temp2 < 50) && (temp3 < 50)){
		check_awb_data=1;
	} else{
		check_awb_data=0;
	}
#endif
	printk("[ov5675 otp read]rg_h = 0x%x, rg_l = 0x%x, bg_h = 0x%x, bg_l = 0x%x\n", 
		read_cmos_sensor(addr + 1), ((temp>>6) & 0x03), 
		read_cmos_sensor( addr + 2), ((temp>>4) & 0x03));
	printk("[ov5675 otp read]rg = 0x%x, bg = 0x%x,g_rg = 0x%x,g_bg = 0x%x\n", 
		(*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio,
		(*otp_ptr).g_rg_ratio,(*otp_ptr).g_bg_ratio);

	for(i=0x7010; i<=0x7041; i++) {
		write_cmos_sensor(i,0); // clear OTP buffer, recommended use continuous write to accelarate
	}

	//set 0x5002[3] to \931\94
	temp1 = read_cmos_sensor(0x5001);
...
// g_rg:golden_rg ratio. g_bg:golden_bg ratio.
static int apply_otp(struct otp_struct *otp_ptr)
{
	int rg, bg, g_rg, g_bg, R_gain, G_gain, B_gain, Base_gain;

	printk("[ov5675 otp write]rg = 0x%x, bg = 0x%x\n", (*otp_ptr).rg_ratio, (*otp_ptr).bg_ratio);
	// apply OTP WB Calibration
	if ((*otp_ptr).flag & 0x40) {
		rg = (*otp_ptr).rg_ratio;
		bg = (*otp_ptr).bg_ratio;
//		g_rg = (*otp_ptr).g_rg_ratio;
//		g_bg = (*otp_ptr).g_bg_ratio;
		g_rg = Golden_RG;
		g_bg = Golden_BG;

		printk("[ov5675 otp write]g_rg = 0x%x, g_bg = 0x%x\n", g_rg, g_bg);
		...
		printk("[ov5675 otp write]rg=0x%x, bg=0x%x, R_gain=0x%x, B_gain=0x%x, G_gain=0x%x\n", rg,bg,R_gain,B_gain,G_gain);
...
{
	printk("OV5675_Sensor_Init_2lane E\n");
	//OV5675R1A_AM01d 
...
#ifdef OV5675_OTP_CALI
	read_otp(&ov5675_otp_data);
	apply_otp(&ov5675_otp_data);
#endif
...
{
	printk(" OV5675PreviewSetting_2lane enter\n");
...
{
	printk("OV5675CaptureSetting_2lane enter! currefps:%d\n",currefps);
...
{
	printk("normal_video_setting Enter! currefps:%d\n",currefps);
	//preview_setting();
