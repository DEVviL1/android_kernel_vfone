...
static int AF_Open(struct inode *a_pstInode, struct file *a_pstFile)
{
   ...
   /*Add by jinlin.Hu for set dw9800 to active mode.Start*/
   i4RetValue = i2c_master_send(g_pstAF_I2Cclient, puSendCmd1, 2);
   if (i4RetValue < 0) {
	printk("[camera AF] Set dw9800 to active mode failed! errcode = %ld\n", i4RetValue);
   } else {
	printk("[camera AF] Set dw9800 to active mode sucess!\n");
	msleep(1);
   }
   /*Add End.*/
   printk("DW9800W open");
