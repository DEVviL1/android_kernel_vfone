...
/*BQ24296m REG00 IINLIM[2:0]*/
const unsigned int INPUT_CS_VTH[] = {
	10000, 15000, 50000, 90000, 
...
static unsigned int charging_hw_init(void *data)
{
	...
	bq24296m_dpdm_en(0x0);						/* disable DPDM detection */

	bq24296m_set_wd_timer(0x1);					/* enable  watch dog 40 secs 0x1 */
	bq24296m_safety_timer_en(0x1);				/* enable charging timer safety timer */
	bq24296m_set_fast_chg_timer(0x2);			/* charging timer 12h */

	bq24296m_set_boostv(0x1);					/* boost voltagte 4.614V */
	bq24296m_set_boost_lim(0x1);				/* boost current limit 1.5A */

	/* set boost mode temperature monitor threshold */
	bq24296m_set_bcold(0x0);					/* Bclod,  76% of REGN or -10.C w/ 103AT thermistor )*/
	bq24296m_set_bhot(0x0);						/* Bhot, 33% of REGN or 55.C w/ 103AT thermistor */
	
	bq24296m_set_thermal_reg(0x3);				/* thermal 120 default */

	bq24296m_set_sys_min(0x5);					/* System min voltage default 3.5V */

	/*PreCC mode */
	bq24296m_set_iprechg(0x2);					/* precharge current 256mA */
	bq24296m_set_batlowv(0x1);					/* precharge2cc voltage,BATLOWV, 3.0V */
	/*CC mode */	
	bq24296m_set_ichg(0x18);					/* fast charge current 2048mA */
	/*CV mode */
	bq24296m_set_vreg(0x38);                	/* charge voltage 4.4V (default 4.208V) */	
	bq24296m_set_vrechg(0x0);					/* recharge voltage@VRECHG=CV-100MV */
	bq24296m_chg_term_en(0x1);					/* enable charge termination */
	bq24296m_set_iterm(0x0);					/* termianation current 128mA */
	/*Vbus current limit */
	bq24296m_set_iinlim(0x6);					/* input current limit, IINLIM, 2A */
	 /*DPM*/ 
	bq24296m_set_vindpm(0x9);					/* input voltage limit, VINDPM = 3.88+ code x 0.08 =4.6V */
...
static unsigned int charging_set_cv_voltage(void *data)
{
	...
	bq24296m_set_vreg(register_value);
...
static unsigned int charging_set_current(void *data)
{
	...
	bq24296m_set_ichg(register_value);
...
static unsigned int charging_set_input_current(void *data)
{
	...
	bq24296m_set_iinlim(register_value);
...
static unsigned int charging_get_charging_status(void *data)
{
	...
	reg_value = bq24296m_get_chrg_state();
...
static unsigned int charging_reset_watch_dog_timer(void *data)
{
	...
	bq24296m_wdt_reset(0x1);	/* reset watchdog timer */
...
#if 0
	bq24296m_pumpx_up(increase);
	battery_log(BAT_LOG_FULL, "Pumping up adaptor...");

#else
	if (increase == KAL_TRUE) {
		bq24296m_set_ichg(0x0);	/* 64mA */
		msleep(85);

		bq24296m_set_ichg(0x8);	/* 512mA */
...
static unsigned int charging_set_hiz_swchr(void *data)
{
	...
	en = *(unsigned int *) data;
	bq24296m_set_en_hiz(en);

	return status;
}

static unsigned int charging_register_reset(void *data)
{
	unsigned int status = STATUS_OK;
	
	battery_log(BAT_LOG_CRTI, "[BQ24296m]Reset register to default.\n");
	bq24296m_register_reset(0x1);
