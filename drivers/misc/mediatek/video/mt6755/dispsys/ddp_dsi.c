	...
	unsigned char *para_list, unsigned int count)
{
	...
	if (packet_type == 0x1A || packet_type == 0x1C) {
		...
		DISPDBG("[ESD]packet_type=0x%x,recv_data_cnt = %d\n",
		...
		if (0 != DSI_REG[i]->DSI_MODE_CTRL.MODE) {
			...
			if (queue_size > 1) {	/* long packet */
				...
				for (j = 0; j < queue_size - 1; j++) {
					OUTREG32(&DSI_VM_CMD_REG[0]->data[j],AS_UINT32((pdata + j + 1)));
				}
			} else {
				vm_cmdq.LONG_PKT = 0;
				vm_cmdq.CM_DATA_ID = ((pdata[0] >> 8) & 0xFF);
			...
			if (force_update) {
				...
				*/
				DSI_EnableVM_CMD(module, cmdq);//modify by jane for 1691146 [CMDQ][ERR]
			}
#endif
		} else {
...
/*shine plus add by zhuqiang for pr949448 begin */
...
#endif
#endif
/*shine plus add by zhuqiang for pr949448 end */
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			...
			/* /default 0xB0 is used to distinguish DCS cmd or Gerneric cmd, is that Right??? */
			t0.Data_ID =(t0.Data0 <0xFF) ? DSI_DCS_READ_PACKET_ID : DSI_GERNERIC_READ_LONG_PACKET_ID;
			t0.Data1 = 0;
			...
			if (dsi_i == 0) {
				...
				DSI_OUTREGBIT(cmdq_trigger_handle, DSI_INT_STATUS_REG,DSI_REG[dsi_i]->DSI_INTSTA, RD_RDY, 0x00000000);
			}
			/* 2. save RX data */
			if (hSlot[0] && hSlot[1] && hSlot[2] && hSlot[3]) {//modify by llf for 1353380
		...
		for (i = 0; i < ESD_CHECK_NUM; i++) {
			if (dsi_params->lcm_esd_check_table[i].cmd == 0)
				break;
			DISPCHECK("[DSI]enter cmp i=%d cmd=0x%x\n", i,dsi_params->lcm_esd_check_table[i].cmd);

			/* read data */
			if (hSlot[0] && hSlot[1] && hSlot[2] && hSlot[3]) {//modify by llf for 1353380
				/* read from slot */
				//DISPCHECK("[ESD_LLF] read from hSlot!\n");
				...
				DISPCHECK("[ESD_LLF] the hSlot create failed!!\n");//modify by llf for defect 
				continue;
/*				if (i == 0) {
					DSI_OUTREG32(NULL, &read_data[0],AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA0));
					DSI_OUTREG32(NULL, &read_data[1],AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA0));
					DSI_OUTREG32(NULL, &read_data[2],AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA0));
					DSI_OUTREG32(NULL, &read_data[3],AS_UINT32(&DSI_REG[dsi_i]->DSI_RX_DATA0));
				}
*/			}
			//DISPDBG("[ESD_DSI]enter cmp read_data0 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
			//     read_data[0].byte0, read_data[0].byte1, read_data[0].byte2,read_data[0].byte3);
			DISPDBG("[ESD_DSI]enter cmp read_data1 byte0=0x%x byte1=0x%x byte2=0x%x byte3=0x%x\n",
...
*/
			ret = DSI_read_cmp(i, read_data, dsi_params->lcm_esd_check_table[i].para_list,dsi_params->lcm_esd_check_table[i].count);
			if (ret){  /*0 esd pass ,1 esd fail */
				break;
			}
		}
	} else if (state == CMDQ_ESD_ALLC_SLOT) {
		/* create 3 slot for max 3 times*/
		unsigned int t = 0;
		unsigned int n = 0;
		for (t = 0;t < 3; t++){
			...
			if (hSlot[0] && hSlot[1] && hSlot[2] && hSlot[3]){//modify by llf for 1353380
				DISPCHECK("[ESD_LLF] Creat the slot succesful!\n");
				break;
			}
		}
		if(t==2){
			DISPCHECK("[ESD_LLF] Creat the slot failed!\n");
			ret = -1;//faild
		}
	} else if (state == CMDQ_ESD_FREE_SLOT) {
		unsigned int h = 0;
		//DISPCHECK("[ESD_LLF] free the slot!\n");
		for (h = 0; h < 4; h++){
			if (hSlot[h]) {
