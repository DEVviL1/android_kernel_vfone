...
/* Added by [peng.tian.hz] at [2015.12.03] for VMCH hold begin */
#if defined(CONFIG_SHINE_PLUS) || defined(CONFIG_SHINE_VDF)
	if (of_find_property(np, "msdc-sd-need-power", &len))
		host->hw->flags |= MSDC_SD_NEED_POWER;
#endif
/* Added by [peng.tian.hz] at [2015.12.03] for VMCH hold end qiaozhen.li.cd*/
