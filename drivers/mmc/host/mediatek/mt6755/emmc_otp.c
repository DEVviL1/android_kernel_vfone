...
#define TRACE_REGION "/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/proinfo"
...
#endif
//add begin  by cansong.li for task 1170621
	OTP_IMEI_Write(BufferPtr);
//add end  by cansong.li for task 1170621
	/* check parameter */
	l_addr = emmc_otp_start();
	l_otp_size = emmc_get_wp_size();
	if (blk_offset > (l_otp_size >> 9))
